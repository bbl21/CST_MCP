...
def clear_project_object():
    """清除当前project对象"""
    global _current_project, _current_fullpath, global_parameters, call_counts, defined_materials, _picked_profile_entities, _open_des_list
    _current_project = None
    _current_fullpath = None
...
def quit_cst(project_path: str = None, force: bool = False):
    ...
    script_path = os.path.join(os.path.dirname(__file__), "tools", "close_cst_by_name.ps1")

    if force or not project_path:
        args = ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", script_path, "-Force"]
    else:
        project_name = os.path.splitext(os.path.basename(project_path))[0]
        args = ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", script_path, "-ProjectName", project_name]
...
def define_brick(name: str, component: str, material: str,
    ...
    VBA_code = [
        ...
        f"    .Xrange {x_min}, {x_max}", f"    .Yrange {y_min}, {y_max}",
        f"    .Zrange {z_min}, {z_max}", "    .Create", "End With"
    ]
...
- 沿Y轴：define_cylinder(name="cyl", component="comp", material="PEC", outer_radius=5, inner_radius=0, axis="y", range_min=-10, range_max=10, x_center=0, z_center=2)
    ...
    VBA_code = [
        "With Cylinder", "    .Reset",
        f"    .Name \"{name}\"", f"    .Component \"{component}\"", f"    .Material \"{material}\"",
        f"    .OuterRadius {outer_radius}", f"    .InnerRadius {inner_radius}",
        f"    .Axis \"{axis}\"", f"    .{range_param}",
        f"    {center1_vba} {center1}", f"    {center2_vba} {center2}",
...
- 沿Y轴：define_cone(name="cone", component="comp", material="PEC", bottom_radius=5, top_radius=15, axis="y", range_min=0, range_max=100, x_center=0, z_center=0)
    ...
    VBA_code = [
        "With Cone", "    .Reset",
        f"    .Name \"{name}\"", f"    .Component \"{component}\"", f"    .Material \"{material}\"",
        f"    .BottomRadius {bottom_radius}", f"    .TopRadius {top_radius}",
        f"    .Axis \"{axis}\"", f"    .{range_param}",
        f"    {center1_vba} {center1}", f"    {center2_vba} {center2}",
        f"    .Segments \"{segments}\"", "    .Create", "End With"
    ]
...
def boolean_subtract(target: str, tool: str):
    ...
    if target_comp and tool_comp and target_comp != tool_comp:
        return {"status": "error", "message": f"目标和工具必须在同一组件中: {target} vs {tool}"}
    ...
    try:
        project.modeler.add_to_history(f"boolean subtract shapes: {full_target} - {full_tool}", sCommand)
        return {"status": "success", "message": f"Boolean subtract {full_target} - {full_tool} completed"}
    except Exception as e:
...
def boolean_add(shape1: str, shape2: str):
    ...
    if comp1 and comp2 and comp1 != comp2:
        return {"status": "error", "message": f"两个实体必须在同一组件中: {shape1} vs {shape2}"}
    ...
    try:
        project.modeler.add_to_history(f"boolean add shapes: {full_shape1} + {full_shape2}", sCommand)
        return {"status": "success", "message": f"Boolean add {full_shape1} + {full_shape2} completed"}
    except Exception as e:
...
def boolean_intersect(shape1: str, shape2: str):
    ...
    try:
        project.modeler.add_to_history(f"boolean intersect shapes: {full_shape1} and {full_shape2}", sCommand)
        return {"status": "success", "message": f"Boolean intersect {full_shape1} & {full_shape2} completed"}
    except Exception as e:
...
def boolean_insert(shape1: str, shape2: str):
    ...
    try:
        project.modeler.add_to_history(f"boolean insert shapes: {full_shape1} insert {full_shape2}", sCommand)
        return {"status": "success", "message": f"Boolean insert {full_shape1} insert {full_shape2} completed"}
    except Exception as e:
...
@mcp.tool()
def define_loft(name: str, component: str, material: str,
...
define_loft(name="loft1", component="component1", material="PEC", tangency=0, minimize_twist=True)
    ...
    VBA_code_loft = [
        "With Loft", "    .Reset",
        f"    .Name \"{name}\"", f"    .Component \"{component}\"", f"    .Material \"{material}\"",
        f"    .Tangency \"{tangency}\"", f"    .Minimizetwist \"{'true' if minimize_twist else 'false'}\"",
        "    .CreateNew", "End With"
    ]
    ...
    for entity in _picked_profile_entities:
        ...
        project.modeler.add_to_history(f"Delete profile:{entity['name']}", sCommand_delete)
        deleted_entities.append(entity['name'])
    _picked_profile_entities.clear()

    if deleted_entities:
        return {"status": "success", "message": f"Loft {name} 执行成功（已删除辅助轮廓: {', '.join(deleted_entities)}）"}
...
@mcp.tool()
def create_loft_sweep(name: str, component: str, material: str,
    ...
    project.modeler.add_to_history(f"Create Profile1:{profile1_name}", sCommand_profile1)
    ...
    project.modeler.add_to_history(f"Create Profile2:{profile2_name}", sCommand_profile2)
    ...
    project.modeler.add_to_history(f"Pick face profile2:{profile2_name}", sCommand_pick2)
    ...
    project.modeler.add_to_history(f"Pick face profile1:{profile1_name}", sCommand_pick1)
    ...
    return {"status": "success", "message": f"Loft sweep {name} 创建成功（临时轮廓已删除）"}


@mcp.tool()
def create_hollow_sweep(name: str, component: str, material: str,
                        x_min1: float, x_max1: float, y_min1: float, y_max1: float, z1: float,
                        x_min2: float, x_max2: float, y_min2: float, y_max2: float, z2: float,
    ...
    project.modeler.add_to_history(f"Pick:{outer_p2}", f'Pick.PickFaceFromId \"{component}:{outer_p2}\", \"1\"')
    project.modeler.add_to_history(f"Pick:{outer_p1}", f'Pick.PickFaceFromId \"{component}:{outer_p1}\", \"1\"')
    ...
    project.modeler.add_to_history(f"Loft:{outer_loft}", line_break.join(VBA_code_outer))

    # ========== 4. 内腔 Loft ==========
    # Pick inner_p2 first (end), then inner_p1 (start)
    project.modeler.add_to_history(f"Pick:{inner_p2}", f'Pick.PickFaceFromId \"{component}:{inner_p2}\", \"1\"')
    project.modeler.add_to_history(f"Pick:{inner_p1}", f'Pick.PickFaceFromId \"{component}:{inner_p1}\", \"1\"')
    ...
    project.modeler.add_to_history(f"Loft:{inner_loft}", line_break.join(VBA_code_inner))
    ...
    return {"status": "success", "message": f"中空放样 {name} 创建成功（临时轮廓已删除）"}
...
@mcp.tool()
def define_units(length_unit: str = "mm", frequency_unit: str = "GHz",
                voltage_unit: str = "V", resistance_unit: str = "Ohm",
                inductance_unit: str = "nH", temperature_unit: str = "degC",
                time_unit: str = "ns", current_unit: str = "A",
                conductance_unit: str = "S", capacitance_unit: str = "pF"):
    """定义单位系统"""
    global line_break
    project_data = get_project_object()
    if not project_data:
    ...
    return {"status": "success", "message": f"Units defined: {length_unit}, {frequency_unit}"}
...
def rename_entity(old_name: str, new_name: str):
    ...
    return {"status": "success", "message": f"Entity renamed from {old_name} to {new_name}"}
...
def define_polygon_3d(name: str, curve: str, points: list):
    ...
    try:
        ...
        return {"status": "success", "message": f"3D polygon {name} created with {len(points)} points"}
    except Exception as e:
        return {"status": "error", "message": f"3D多边形创建失败: {str(e)}", "command": sCommand}

@mcp.tool()
def define_analytical_curve(name: str, curve: str, law_x: str, law_y: str, law_z: str,
                           param_start: str, param_end: str):
    """定义解析曲线
    ...
    try:
        ...
        return {"status": "error", "message": f"解析曲线创建失败: {str(e)}", "command": sCommand}

@mcp.tool()
def define_extrude_curve(name: str, component: str, material: str, curve: str,
                        thickness: Union[float, str], twist_angle: float = 0.0,
                        taper_angle: float = 0.0, delete_profile: bool = True,
                        ):
    """定义拉伸曲线（从曲线创建实体）
    ...
    try:
        ...
        return {"status": "error", "message": f"拉伸曲线创建失败: {str(e)}", "command": sCommand}

@mcp.tool()
def transform_shape(shape_name: str, transform_type: str, center_x: str = "0",
                   center_y: str = "0", center_z: str = "0",
                   plane_normal_x: str = "0", plane_normal_y: str = "1", plane_normal_z: str = "0",
                   angle_x: str = "0", angle_y: str = "0", angle_z: str = "0",
                   multiple_objects: bool = True, group_objects: bool = False,
                   repetitions: int = 1, destination: str = ""):
    """变换形状（镜像或旋转）
    ...
    try:
        ...
        return {"status": "success", "message": f"Shape {shape_name} transformed with {vba_transform}"}
    except Exception as e:
        return {"status": "error", "message": f"变换形状失败: {str(e)}", "command": sCommand}

@mcp.tool()
def transform_curve(curve_name: str, transform_type: str, center_x: str = "0",
                   center_y: str = "0", center_z: str = "0",
                   plane_normal_x: str = "0", plane_normal_y: str = "1", plane_normal_z: str = "0",
                   multiple_objects: bool = True, group_objects: bool = False):
    """变换曲线（仅镜像）"""
...
def change_material(shape_name: str, material: str):
    ...
    return {"status": "success", "message": f"Material of {shape_name} changed to {material}"}

@mcp.tool()
def set_entity_color(shape_name: str, use_individual_color: bool = True,
                     r: int = 192, g: int = 192, b: int = 192):
    """设置实体的颜色"""
    project_data = get_project_object()
    if not project_data:
        return {"status": "error", "message": "当前没有活动的项目"}
...
@mcp.tool()
def define_solver(stimulation_port: str = "All", stimulation_mode: str = "All",
                 steady_state_limit: float = -40, mesh_adaption: bool = False,
                 auto_norm_impedance: bool = True, norming_impedance: float = 50,
                 calculate_modes_only: bool = False, s_para_symmetry: bool = False,
                 store_td_results: bool = False, run_discretizer_only: bool = False,
                 full_deembedding: bool = False, superimpose_plw: bool = False,
    ...
    VBA_code = [
        ...
        'With Solver',
        '     .Method "Hexahedral"', f'     .CalculationType "TD-S"',
        f'     .StimulationPort "{stimulation_port}"', f'     .StimulationMode "{stimulation_mode}"',
        f'     .SteadyStateLimit "{steady_state_limit}"', f'     .MeshAdaption "{"True" if mesh_adaption else "False"}"',
        f'     .AutoNormImpedance "{"True" if auto_norm_impedance else "False"}"', f'     .NormingImpedance "{norming_impedance}"',
...
@mcp.tool()
def set_solver_acceleration(use_parallelization: bool = True, max_threads: int = 1024,
                           max_cpu_devices: int = 2, remote_calc: bool = False,
                           use_distributed: bool = False, max_distributed_ports: int = 64,
                           distribute_matrix: bool = True, mpi_parallel: bool = False,
                           auto_mpi: bool = False, hardware_accel: bool = True,
                           max_gpus: int = 4):
    """设置求解器并行计算加速"""
    project_data = get_project_object()
    if not project_data:
...
@mcp.tool()
def set_farfield_monitor(start_freq: float, end_freq: float, step: float = 1,
                         subvolume_x_min: float = -105, subvolume_x_max: float = 105,
                         subvolume_y_min: float = -105, subvolume_y_max: float = 105,
                         subvolume_z_min: float = 0, subvolume_z_max: float = 445,
    ...
    return {"status": "success", "message": f"Farfield monitor set: {start_freq}-{end_freq} GHz"}

@mcp.tool()
def set_efield_monitor(start_freq: float, end_freq: float, step: float = 1,
                       dimension: str = "Volume", subvolume_x_min: float = -105,
                       subvolume_x_max: float = 105, subvolume_y_min: float = -105,
                       subvolume_y_max: float = 105, subvolume_z_min: float = 0,
    ...
    return {"status": "success", "message": f"E-field monitor set: {start_freq}-{end_freq} GHz"}

@mcp.tool()
def set_background_with_space(x_min_space: float = 30, x_max_space: float = 30,
                             y_min_space: float = 30, y_max_space: float = 30,
                             z_min_space: float = 50, z_max_space: float = 100):
    """设置背景空间"""
    project_data = get_project_object()
    if not project_data:
        return {"status": "error", "message": "当前没有活动的项目"}
...
def create_mesh_group(group_name: str, items: list):
    ...
    return {"status": "success", "message": f"Mesh group {group_name} created with {len(items)} items"}
...
def set_mesh_fpbavoid_nonreg_unite(enable: bool = True):
    ...
    return {"status": "success", "message": f"Mesh.FPBAAvoidNonRegUnite set to {enable}"}
...
def set_mesh_minimum_step_number(num_steps: int = 5):
    ...
    return {"status": "success", "message": f"Mesh.MinimumStepNumber set to {num_steps}"}
...
def activate_post_process_operation(operation: str, enable: bool = True):
    ...
    return {"status": "success", "message": f"PostProcess1D operation '{operation}' {'activated' if enable else 'deactivated'}"}
...
@mcp.tool()
def export_farfield(farfield_name: str, frequency: str, file_path: str, plot_mode: str = "Efield"):
    """导出远场方向图数据（ASCII格式）
