...
```
skill 目录（只读）                   工作区 / 测试区（可写）
┌──────────────────┐               ┌─────────────────────┐
│ <skill-root>/    │               │ tasks/task_xxx/     │
│  ├ scripts/      │               │  ├ .venv/  ← uv 创建 │
│  │ ├ cst_runtime/ │ ← 模块源码   │  ├ pyproject.toml   │
│  │ └ *_cli.py    │ ← 入口       │  └ runs/            │
│  └ SKILL.md      │               └─────────────────────┘
└──────────────────┘
```

- 模块源码**只存在 skill 目录**，工作区只有配置文件 `.venv` 和 `pyproject.toml`
- `health-check --auto-fix` 在当前工作目录（如测试区）运行 `uv sync` → 创建 `.venv`
- `.venv` 只能在其所在目录下用 `uv run` 激活——离开该目录就无法访问
- **严禁**从 skill 目录复制 `cst_runtime/` 到工作区；所有命令通过 skill 入口运行

### 两种入口模式

| 模式 | 命令 | 条件 | 原理 |
|------|------|------|------|
| **引导** | `python <skill-root>\scripts\cst_runtime_cli.py` | 首次运行，零配置 | 入口脚本自动将 `scripts/` 加入 `sys.path`，从 skill 目录加载模块 |
| **生产** | `uv run python -m cst_runtime` | 先在工作区跑过 `health-check --auto-fix` | `uv sync` 在工作区创建 `.venv`，脚本入口添加了包的查找路径，模块在该 uv 环境中可解析 |

> 引导模式在任何目录都可以运行（因为入口脚本自己调整 sys.path）。生产模式只能在跑过 `health-check` 的工作目录下用 `uv run`。
